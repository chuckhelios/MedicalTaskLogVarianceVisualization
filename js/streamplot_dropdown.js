...
var graph = d3.csv(csvpath, function(data) {
  ...
  if (data[0].hasOwnProperty('dep_id')){
    //create dropdown menu for selecting department
    var unique = {};
    var selectDept = [{"value": "overview", "text": "Overview", "selected": true}];
    for( var i in  data){
      if( typeof(unique[data[i].dep_id]) == "undefined"){
        selectDept.push({"value":data[i].dep_id, "text":data[i].dep_id});
      }
      unique[data[i].dep_id] = 0;
    }
    ...
    subtitleTxt = "DepartmentID: " + dropdown.value.toString();
